/**
 * connector-config backend entrypoint.
 *
...
export const CAPABILITY_ID = 'connector-config';

/** Register health checks + job handlers with the core singletons. */
export function register(): void {
  health.register(CAPABILITY_ID, 'implemented', async () => ({
    state: 'unknown',
    detail: 'connector-config: no runtime service yet — capability is contracts-first',
  }));
}

...
|---|---|---|---|---|
| node-a | RTX (per `node-a_ollama_store.md`, hosts qwq/deepseek-r1/qwen2.5vl 32B models on G:) | NVIDIA/CUDA | high | Heavy inference; cloud-deep workloads |
| node-b | RTX | NVIDIA/CUDA | high | Mirror for racing + transcode |
| node-c | AMD Tahiti (`node-c_machine.md`) | AMD/ROCm | mid | Smaller models + transcode (NVENC unavailable) |
...
- **Network latency** (node-a 0ms > node-b 1ms direct-link > node-c 2ms LAN, weight 0.2)
